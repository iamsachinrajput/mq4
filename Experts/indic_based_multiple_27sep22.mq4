...
int current_session_orders_count=0;
       ...
       //ssl gann indicator 
        ...
        double skind_rsi_lastvalue,skind_rsi_currentvalue,skind_rsi_currentvalue1;
...
//+------------------------------------------------------------------+
    ...
    string buy_sell_with_rsi()
        {
        
             skind_rsi_lastvalue=skind_rsi_currentvalue;
             skind_rsi_currentvalue=iRSI(Symbol(),Period(),skind_rsi_input1_PERIOD,,0);
             skind_rsi_currentvalue1=iCustom(Symbol(),Period(),skind_rsi,
                                                     skind_rsi_input1_PERIOD,
                                                     skind_rsi_output1,1);
             
            printf("rsi="+   skind_rsi_currentvalue + " rsi1 = " +    skind_rsi_currentvalue1);
                                              
            string returnvalue="none";
            
            if(skind_rsi_currentvalue>=75)
                returnvalue="sell";
            else if(skind_rsi_currentvalue<=25)
                returnvalue="buy";
      ...
      if(barcountnow>=lastbarscount+barsgap || total_orders_open_live==0  ) 
       {
        ...
        if(action_decided=="buy" )
            draw_vline("buystart"+barcountnow,"buystart",closeprice,clrGreen,2);
               else if(action_decided=="sell" )
                  draw_vline("sellstart"+barcountnow,"sellstart",closeprice,clrRed,2);
