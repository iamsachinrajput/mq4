...
extern bool no_new_trades=False;
...
//#include "peak_ea_inputs_btcusd.mqh"
//#include "peak_ea_inputs_xauusd.mqh"
//#include "peak_ea_inputs_eurusd.mqh"
#include "peak_ea_inputs_gbpusd.mqh"
//#include "peak_ea_inputs_all.mqh"
...
/// for XAUUSD usdc exness : 
 /*
extern double filter_input_usd = 20;
extern double SL_for_pendingorders_usd = .2;  //this is for our BUY/SELL STOP SL 2.3 pips modify, can not put 200 pips, will lose !!!
extern  double sl_trail_active_orders_input_usd = .1; // not used !!! 30
extern  double noprofit_sl_trail_active_orders_input_usd = .1;  // 20 
extern  double withprofit_sl_trail_active_orders_input_usd = .1;  // 30
extern  double distance_for_pending_order_usd = .11;    // 20 will cause order130 error  //23
...
//################  Main calculation to trade decision check if high movement is there 
   ...
   int total_orders_open_pending_buy=0;
   int total_orders_open_pending_sell=0;
   int total_orders_open_active_buy=0;
   int total_orders_open_active_sell=0;
      ...
      if (OrderSelect(order_position_for_selecting, SELECT_BY_POS, MODE_TRADES)) 
      {
         if (OrderMagicNumber() == MagicNumber  && Symbol()==OrderSymbol()) 
         {
            ...
            if (OrderSymbol() == Symbol()) 
            {
                 ...
                 SL_points_touse=MathMax((1.5*spread_calculated/Point),MathMax(MathMax(1,GS_allowed_SL_size_for_symbol),sl_for_pendingorders)) ; 
            
               int current_ticket_number=OrderTicket();
               total_orders_open_live++;
               TradeComment="SP"+(spread_calculated)+"sl"+(SL_points_touse*Point)+"_PR"+profit_total+"_PK"+(highlow_diff/GS_point);
               ...
               case OP_BUY:
                 ...
                 TotalProfit = (Bid - OrderOpenPrice()) /Point/Dfactor_basedondigits;
                  ...
                  SL_tobeused_inorder = NormalizeDouble(Bid - normalized_sl_trail_activeorders-spread_calculated, GS_digits);  // Bid - sl_trail_active_orders_input
                  //if(TotalProfit >0)
                  SL_tobeused_inorder = NormalizeDouble(Bid - normalized_sl_trail_activeorders, GS_digits);  // Bid - sl_trail_active_orders_input
                  ...
                  {
                  Print("Modify Buy success : "+TradeComment+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                     break;
                  }
               ...
               case OP_SELL:
                 ...
                 TotalProfit = (OrderOpenPrice()- Ask)/Point/Dfactor_basedondigits;
                  ...
                  SL_tobeused_inorder = NormalizeDouble(Ask + normalized_sl_trail_activeorders+spread_calculated, GS_digits);
                  //if(TotalProfit >0)
                  SL_tobeused_inorder = NormalizeDouble(Ask + normalized_sl_trail_activeorders, GS_digits);
                  ...
                  if(boolvar_modify_done) 
                  {
                  Print("Modify Sell success : "+TradeComment+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                     break;
                  }
                                    error_code = GetLastError();
                  error_details = ErrorDescription(error_code);
               ...
               case OP_BUYSTOP:
                  total_orders_open_pending_buy++;

                  price_tobeused = NormalizeDouble(Ask + normalized_distance_for_stop_order+spread_calculated, GS_digits);  // Ask + distance_for_pending_order
                  ...
                  if(boolvar_modify_done) 
                  {
                  Print("Modify Buystop success :"+TradeComment+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                     break;
                  }
                  error_code = GetLastError();
                  error_details = ErrorDescription(error_code);
                  ...
                  break;
               case OP_SELLSTOP:
                  total_orders_open_pending_sell++;
                  current_order_price = NormalizeDouble(OrderOpenPrice(), GS_digits);
                  price_tobeused = NormalizeDouble(Bid - normalized_distance_for_stop_order-spread_calculated, GS_digits);
                  ...
                  if(boolvar_modify_done) 
                  {
                  Print("Modify sell stop success :"+TradeComment+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                     break;
                  }
                  error_code = GetLastError();
                  error_details = ErrorDescription(error_code);
      ...
      */
      ...
      //printf("########faCHECK== Traded lots="+ total_traded_lots_tillnow+"/"+lots_in_this_session+ " Brokerage="+brokerage_total +"/"+brokerage_current+" Profit = " + (current_equity-equity_at_start )+"/"+(current_equity-equity_at_start -brokerage_total) + " equity from " +equity_at_start+"/"+total_invested_money+"/"+ equity_allocated_touse +" to "+ current_equity+"/"+(current_equity-brokerage_total) +"/"+ usable_equity + "------#####--- "  );
       ...
       //TradeComment="PEK:"+(highlow_diff/GS_point)+"_PR="+profit_total+"_SPR="+(spread_calculated/GS_point);
        SL_points_touse=MathMax((1.5*spread_calculated/Point),MathMax(MathMax(1,GS_allowed_SL_size_for_symbol),sl_for_pendingorders)) ; 
       TradeComment="SP"+(spread_calculated)+"sl"+(SL_points_touse*Point)+"_PR"+profit_total+"_PK"+(highlow_diff/GS_point);
       //TradeComment="SP"+(spread_calculated)+"ds"+(normalized_distance_for_stop_order+spread_calculated)+"_PR"+profit_total+"_PK"+(highlow_diff/GS_point);
       
      if ( ( buyorsell == -1 || buyorsell == 2 ) && ( (total_orders_open_active_buy+total_orders_open_pending_buy ) < MaxTrades/2) ) 
      {
         price_tobeused = NormalizeDouble(Ask + normalized_distance_for_stop_order+spread_calculated, GS_digits);
      ...
      if ( (buyorsell == 1 || buyorsell == 2 ) && ( (total_orders_open_active_sell+total_orders_open_pending_sell ) < MaxTrades/2) )
      {
          price_tobeused = NormalizeDouble(Bid - normalized_distance_for_stop_order-spread_calculated, GS_digits);
          //SL_tobeused_inorder = NormalizeDouble(price_tobeused +  3 *sl_for_pendingorders  * Point, GS_digits);
          SL_tobeused_inorder = NormalizeDouble(price_tobeused +  SL_points_touse  * Point, GS_digits);
        ...
        //SL_tobeused_inorder=0;
         if (tp_input>0)
          ...
          if (ticket_20 <= 0) {
            ...
            {
            //PlaySound("Alert.wav");
            printf("SELLSTOP succes LT: " + DoubleToStr(lot_size_to_trade, GS_step_logvalue)+TradeComment + " OP: " + DoubleToStr(price_tobeused, GS_digits) + " SL: "+DoubleToStr(SL_tobeused_inorder-price_tobeused,GS_digits)+"=" +  DoubleToStr(SL_tobeused_inorder, GS_digits)+" TP:" + TP_tobeused_inorder + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
...
//// Calculations for peaks 
 ...
 //if(profit_total-profit_till_last_peak!=0)
 {
   ...
   + "\n Current Spread: " + ((Ask-Bid)/GS_point  ) +"/"+(maxspread*usd_points)
   ...
   + "   Avg Sprd: " + int(average_spread/usd_points) +" brkg actual:" + (commision_forpip/GS_point) + " spread+comision: " + int(average_spread_withcommision/ usd_points) + "/" + (MaxSpreadPlusCommission_usd)
  ...
  // + "\n Lots:usable eq="+( usable_equity )  +" * leverage="+ AccountLeverage() +" * riskpercent="+ riskpercent + "="+( usable_equity * AccountLeverage() * riskpercent)
   ...
   if(total_orders_open_active==0 && total_orders_open_pending>0 && ispeakactive ==False )
      TimedClosing();
   ...
   for (int i = total - 1; i >= 0; i--) {
    ...
    if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) == TRUE) { 
      ...
      if ( ( OrderType() == OP_SELLSTOP || OrderType() == OP_BUYSTOP) && OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) 
      {
         ...
         {
            //RefreshRates();
            printf("closing the Pending order#"+ OrderTicket()+" due to time over of  "+Minutes_Buy + " Minutes ");
