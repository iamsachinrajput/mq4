...
extern double take_profit_usd_input=2;
...
if( stop_working==False )
{
   ...
   if(  MathMod(TimeMinute(TimeCurrent()),15)==0  ) 
      open_15min=Ask;
...
if( lower_profit_just_close==True  )
   ...
   if(  current_session_profit < (sl_usd*-1) ) 
      {
                  close_all_orders("trail tp Profit="+current_session_profit);
                  update_session_profit();
      }
...
void display_tasks()
{
   ...
   message_display=("price Now : "+Ask+" today open ="+skind_open_currentvalue +" last="+skind_open_currentvalue_last+ " start="+digit2(equity_at_start)+"/"+equity_start_for_this_session+" Pr="+digit2(AccountEquity()-equity_at_start)+ " rt="+TotalrunningProfit+" Eqt="+digit2(AccountEquity()) + " free="+digit2(AccountFreeMargin()) 
   +" Max="+max_equity+" Min="+min_equity
   +" orders="+total_orders_open_active+"/"+total_orders_open_live+"/"+max_orders_active+"/"+max_orders 
   +"\n Crpr="+(TotalrunningProfit)+"/"+(current_session_profit_max) +" Lots active="+active_lots+" pending="+pending_lots
      ...
      if(total_orders_open_live==0 )
       {
        ...
        double downline=(skind_open_currentvalue-(pips_near_open*Point() ));
         ...
         closeprice=iClose(Symbol(),PERIOD_M1,0);
         openprice=iOpen(Symbol(),PERIOD_M1,0);
         //double closeprice=iClose(Symbol(),PERIOD_M15,0);
         //double closeprice=iClose(Symbol(),PERIOD_M15,0);
        
            
            if(closeprice > upline && openprice < upline )
               {  action1=OP_BUY; printf("will do buy :lastaction was "+last_action +" Bid:"+ Bid +">"+ upline+" cp="+closeprice ) ; }
            //if(Ask < selllimitprice  )
            if(closeprice < downline && openprice > downline  )
               {  action1=OP_SELL;printf("will do sell :lastaction was "+last_action +" Ask:"+ Ask +"<"+ downline+" cp="+closeprice   ); }
...
void set_open_line_value()
{
   ...
   if(StringFind( baseline,",dailyopen1,")>=0)
      { skind_open_currentvalue=open_today;
         printf("SK1 daily line set to : "+ open_today ) ;
      }
