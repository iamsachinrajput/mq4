...
// INPUTS 
extern string baseline=",,daily_open,";
extern string baseline_info="monthly_open,open_4hr,open_15min,daily_open,first_candle_open,hourly_open,weekly_open,";
extern double lots_to_start_input=0.01;
extern double sl_usd_input=-1;
extern bool chang_gap_for_martingle=True;
extern double take_profit_usd_input=-1;
extern double set_take_profit_usd_input=1;
extern double per_lot_profit_usd=.6;
extern bool trail_profit_input=True;
extern bool add_extra_on_trail=False;
extern double trail_profit_usd_input=.4;
extern double lots_multiplier=2;
extern int max_orders_depth=10;
...
if( stop_working==False )
{
   ...
   int thisminute=TimeMinute(TimeCurrent());
         ...
         closeprice=iClose(Symbol(),0,0);
         openprice=iOpen(Symbol(),0,0);
...
if( lower_profit_just_close==True  )
   ...
   if(trail_profit==False)
      ...
      if(  current_session_profit > take_profit_usd ) 
         {
         close_all_orders( "TP without trail "+ current_session_profit +" > "+take_profit_usd);
         //update_session_profit();
         }
      }
   else
   ...
   if(  current_session_profit <= (sl_usd) && sl_usd!=0 ) 
      {
                  close_all_orders("SL closing orders as profit "+current_session_profit +" is less then SL"+sl_usd);
                  //update_session_profit();
      }
...
void display_tasks()
{
   ...
   +"\n Crpr="+(TotalrunningProfit)+"("+(current_session_profit_max)+")/"+take_profit_usd_input +" Lots active="+active_lots+" pending="+pending_lots
   ...
   +"\n Maxlots="+max_lot_size+" NextLot="+lot_size_to_trade +" total running_lots="+digit2(total_open_lots)+"(B"+digit2(total_open_lots_buy)+"S"+digit2(total_open_lots_sell)+"="+digit2(MathAbs(total_open_lots_buy-total_open_lots_sell))+") SL="+sl_usd+" TP="+digit2(take_profit_usd)+" MTP="+digit2(take_profit_usd-trail_profit_usd*MathAbs(total_open_lots_sell-total_open_lots_buy) )+" trail_stared="+trail_stared
   +" Gap="+gapinput_pips
   ...
   +"\n Maxlots="+max_lot_size+" NextLot="+lot_size_to_trade +" total running_lots="+digit2(total_open_lots)+"(B"+digit2(total_open_lots_buy)+"S"+digit2(total_open_lots_sell)+"="+digit2(MathAbs(total_open_lots_buy-total_open_lots_sell))+") TP="+digit2(take_profit_usd)+" MTP="+digit2(take_profit_usd-trail_profit_usd)+" trail_stared="+trail_stared
      ...
      if(total_orders_open_live<=max_orders_depth )
       {
        ...
        double downline=sellprice;
            ...
            if(closeprice > buyprice && openprice < buyprice && ( last_action==OP_SELL || total_orders_open_live==0  ))
               {  
                  if(reverse==True)
            ...
            if(closeprice < sellprice && openprice > sellprice && ( last_action==OP_BUY || total_orders_open_live==0  ) )
               {  
                  if(reverse==True)
                     { action1=OP_BUY; printf("will do buy  of "+lot_size_to_trade+" lots lastaction was "+last_action +" Bid:"+ Bid +">"+ buyprice+" cp="+closeprice +" op="+openprice ) ; }
                     else
...
void place_order(int action1 )
{
      ...
      if (action1==OP_BUYSTOP ) 
      {
         ...
         if(set_take_profit_usd_input >0 )
         TP_tobeused_inorder=price_tobeused+set_take_profit_usd_input*Point();
  ...
  // printf("openlinesetting:baseline="+baseline+" searchresult="+StringFind( baseline,",dailyopen1,") + " : " +skind_open_currentvalue+":"+ open_today ) ;
   ...
   if(StringFind( baseline,",first_candle_open,")>=0)
      ...
      if(  total_open_lots==0 ) 
         ...
         daily_first15_min_high=iHigh(NULL,15,1);
         daily_first15_min_low=iLow(NULL,15,1);     
    ...
    if(StringFind( baseline,",daily_open,")>=0)
      { 
      if(  TimeHour(TimeCurrent()) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
     ...
     if(StringFind( baseline,",4hour_open,")>=0)
      { 
      if(  MathMod(TimeHour(TimeCurrent()),4) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 daily line set to : "+ Ask ) ;
         daily_first15_min_high=iHigh(NULL,15,1);
     ...
     if(StringFind( baseline,",8hour_open,")>=0)
      { 
      if(  MathMod(TimeHour(TimeCurrent()),8) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 daily line set to : "+ Ask ) ;
         daily_first15_min_high=iHigh(NULL,15,1);
     ...
     if(StringFind( baseline,",12hour_open,")>=0)
      { 
      if(  MathMod(TimeHour(TimeCurrent()),12) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 daily line set to : "+ Ask ) ;
         daily_first15_min_high=iHigh(NULL,15,1);
   ...
   if(StringFind( baseline,",weekly_open,")>=0)
      { 
      if( DayOfWeek()== 1 && TimeHour(TimeCurrent()) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 daily line set to : "+ Ask ) ;
         daily_first15_min_high=iHigh(NULL,15,1);
    ...
    if(StringFind( baseline,",monthly_open,")>=0)
      { 
      if( TimeDay(TimeCurrent())== 1 && TimeHour(TimeCurrent()) == 0 &&  DayOfWeek()== 1 && TimeHour(TimeCurrent()) == 0 && TimeMinute(TimeCurrent())==15 && TimeSeconds(TimeCurrent())==0) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 daily line set to : "+ Ask ) ;
         daily_first15_min_high=iHigh(NULL,15,1);
   ...
   if(StringFind( baseline,",open_15min,")>=0)
      ...
      if( MathMod(TimeMinute(TimeCurrent()),15) == 0 && TimeSeconds(TimeCurrent())==0 ) 
         {
         skind_open_currentvalue=Ask;
         printf("SK1 open 15min line set to : "+ Ask ) ;
         }
      }
     ...
     if(chang_gap_for_martingle==True)
      {
       ...
       buyprice=daily_first15_min_high +( daily_first15_min_high-daily_first15_min_low)*.05 *total_orders_open_active ;
       sellprice=daily_first15_min_low - ( daily_first15_min_high-daily_first15_min_low)*.05 *total_orders_open_active ;
      }
  ...
  if(daily_first15_min_high==0 || (skind_open_currentvalue_last!=skind_open_currentvalue && skind_open_currentvalue!=0)  )
   ...
   printf("setting new hline : open val : Last="+skind_open_currentvalue_last+" now="+skind_open_currentvalue+" 15minhigh = "+daily_first15_min_high) ;
      if(close_all_on_new_lines==True)
         close_all_orders("closing all for new levels : Loss="+current_session_profit );
