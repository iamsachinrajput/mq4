...
extern double Risk_input = 10;  // org 60
...
extern double StopLoss = 3000;  //this is for our BUY/SELL STOP SL 2.3 pips modify, can not put 200 pips, will lose !!!
extern int target_input_points=0 ; 

extern int Filter_input = 25000;
...
extern double MaxSpread = 2500  ;//6.3 ok !
...
color clr_pending_order_open_buy=clrYellow;
color clr_pending_order_open_sell=clrYellow;
color clr_pending_order_close=clrNONE;
color clr_pending_order_modify=clrNONE;
color clr_confirm_order_open=clrLimeGreen;
color clr_confirm_order_modify_buy=clrAliceBlue;
color clr_confirm_order_modify_sell=clrAliceBlue;
color clr_confirm_order_close=clrGreen; 
...
int init() 
    {
      ...
      equity_seen_min=equity_at_start=current_equity;
        ...
        MaxSpread  = MaxSpread_EU; Filter = Filter_EU;
           ...
           if(Filter_input==0)
            Filter=current_spread*11;
           //Filter=150;
           //GS_point=1;
           //StopLoss=3000;
           StopLoss=MathMax((Ask-Bid)*GS_point*2,StopLoss); 
           SL_points_touse=MathMax(MathMax(1000,GS_allowed_SL_size_for_symbol),StopLoss) ; 
           //MaxSpread  = 5000; 
           //MaxSpreadPlusCommission=5000;
           //Limit=MathMax(GS_allowed_SL_size_for_symbol,600) ; 
           //NoTrade_Profit_Yet_Limit=MathMax(GS_allowed_SL_size_for_symbol,600) ; 
           //InTrade_Profit_Limit=MathMax(GS_allowed_SL_size_for_symbol,2000) ; 
           //Distance_input=MathMax(GS_allowed_SL_size_for_symbol,3000) ; 
           //Limit=MathMax(GS_allowed_SL_size_for_symbol,Limit) ; 
           //NoTrade_Profit_Yet_Limit=MathMax(GS_allowed_SL_size_for_symbol,NoTrade_Profit_Yet_Limit) ; 
           //Distance_input=MathMax(GS_allowed_SL_size_for_symbol,Distance_input) ; 

            if(symbol_name == "BTCUSDc")
              Risk = Risk *100;
           Limit=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,Limit)) ; 
           NoTrade_Profit_Yet_Limit=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,NoTrade_Profit_Yet_Limit)) ; 
           InTrade_Profit_Limit=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,InTrade_Profit_Limit)) ; 
           Distance_input=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,Distance_input)) ; 
           NoTrade_Profit_Yet_Distance=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,NoTrade_Profit_Yet_Distance));
           InTrade_Profit_Distance=MathMax(10,MathMax(GS_allowed_SL_size_for_symbol,InTrade_Profit_Distance));
           }
            ...
            if (OrderSymbol() == Symbol()) 
            {
               ...
               case OP_BUY:
                 ...
                 TotalProfit = (Bid - OrderOpenPrice()) /Point/Dfactor_basedondigits;
                  ...
                  boolvar_modify_done = OrderModify(OrderTicket(), OrderOpenPrice(), SL_tobeused_inorder, TP_tobeused_inorder, 0, clr_confirm_order_modify);
   ...
   {  // New BUY STOP & SELL STOP Orders placed here..
      ...
      */
      ...
      if (buyorsell == -1 || buyorsell == 2 ) 
      {
         ...
         ticket_20 = OrderSend(Symbol(), OP_BUYSTOP, lot_size_to_trade, price_tobeused, G_slippage_264, SL_tobeused_inorder, TP_tobeused_inorder, TradeComment, MagicNumber, 0, clr_pending_order_open);
         if (ticket_20 <= 0) {
            error_code = GetLastError();
            error_details = ErrorDescription(error_code);
      ...
      if (buyorsell == 1 || buyorsell == 2 ) {
        ...
        TP_tobeused_inorder=0;
         if (target_input_points>0)
          ...
          ticket_20 = OrderSend(Symbol(), OP_SELLSTOP, lot_size_to_trade, price_tobeused, G_slippage_264, SL_tobeused_inorder, TP_tobeused_inorder, TradeComment, MagicNumber, 0, clr_pending_order_open);
          if (ticket_20 <= 0) {
            error_code = GetLastError();
            error_details = ErrorDescription(error_code);
                  printf(" #"+ticket_20 + " given_limit_normalized="+given_limit_normalized+ "OP: " + DoubleToStr(price_tobeused, GS_digits) +"/"+(price_tobeused-current_order_price)+ " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) +"/"+(price_tobeused-SL_tobeused_inorder)+ " Bid: " + DoubleToStr(Bid, GS_digits) +"/"+(Bid-SL_tobeused_inorder)+ " Ask: " + DoubleToStr(Ask, GS_digits) +"/"+(Ask-SL_tobeused_inorder) );
...
//// Calculations for peaks 
 ...
 //if(profit_total-profit_till_last_peak!=0)
 {
   total_orders_executed_prev=total_orders_executed;
    ...
    array_of_peaks[peaks_count]=peak_name+"("+profit_in_this_peak +"/"+profit_total +")";
   ...
   + "   Avg Sprd: " + (average_spread/GS_point/10) +" brkg actual:" + (commision_forpip/ GS_point/10 ) + " spread+comision: " + (average_spread_withcommision/ GS_point/10)
  ...
  // + "\n Lots:usable eq="+( usable_equity )  +" * leverage="+ AccountLeverage() +" * riskpercent="+ riskpercent + "="+( usable_equity * AccountLeverage() * riskpercent)
   ...
   +"\n#Limits (L/N/I/D)="+Limit +"/"+NoTrade_Profit_Yet_Limit+"/"+InTrade_Profit_Limit+"/"+Distance_input
   ;
...
bool remove_pending_order(int ticket_number,color clr1)
   {
      ...
      if( OrderDelete(ticket_number,clr_pending_order_close) )
         {
    ...
    if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) == TRUE) { 
      if (OrderType() == OP_BUY && OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber ) 
      {
         if (TimeCurrent() - OrderOpenTime() >= 60 * ExpirationBuyTime && Bid >= OrderOpenPrice() + MinBuyProfit * Point) 
         {
            ...
            TicketClose = OrderClose(OrderTicket(), OrderLots(), Bid, G_slippage_264 , clr_confirm_order_close);
            if (!TicketClose) 
             {
      ...
      if (OrderType() == OP_SELL && OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) 
      {
         if (TimeCurrent() - OrderOpenTime() >= 60 * ExpirationSellTime && Ask <= OrderOpenPrice() - MinSellProfit * Point) 
         {
            ...
            TicketClose = OrderClose(OrderTicket(), OrderLots(), Ask, G_slippage_264, clr_confirm_order_close);
            //---
            if (!TicketClose) {
              //skcomment printf("There was an error closing the order#"+OrderTicket()+". Error is:", GetLastError());
              return;
      ...
      if ( ( OrderType() == OP_SELLSTOP || OrderType() == OP_BUYSTOP) && OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) 
      {
         if (TimeCurrent() - OrderOpenTime() >= 60 * ExpirationSellTime ) 
         {
            ...
            TicketClose = OrderDelete(OrderTicket(),clr_pending_order_close);
            //---
            if (!TicketClose) {
              //skcomment printf("There was an error closing the order#"+OrderTicket()+". Error is:", GetLastError());
              return;
         ...
         if (OrderType() == OP_BUY && (OrderMagicNumber() == MagicNumber)) {
            if (Use_TrailingStep && Bid - OrderOpenPrice() >= ld_52) {
               ...
               if (Bid - OrderStopLoss() >= ld_52 + order_SL_normalized) {
                  result = OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss() + order_SL_normalized, OrderTakeProfit(), 0, clr_confirm_order_modify);
            ...
            if (Use_Set_BreakEven && Bid - OrderOpenPrice() >= ld_68) {
               if (OrderStopLoss() < OrderOpenPrice()) {
                  result= OrderModify(OrderTicket(), OrderOpenPrice(), OrderOpenPrice() + ld_60, OrderTakeProfit(),0, clr_confirm_order_modify);
...
int deinit()
   ...
   profit_till_last_peak=profit_total;
   peak_name_last=peak_name;
   peak_name=""+TimeDayOfYear(TimeCurrent())+TimeHour(TimeCurrent());
   //if(peak_name_last != peak_name )
       peak_details+=peak_name_last+"("+profit_in_this_peak +"/"+profit_till_last_peak +")";
   */
