...
//input datetime licesevaliddate;
 bool send_telgram_message_flag=False;
...
extern double StopLoss_input=0;

 double MinLots = 0.01;
 double MaxLots = 50000.0;//100000.0;
...
extern int Filter_input = 19000;
extern int highlow_diff_level=1;

 int Filter_EU = 130; //org 140
 int Filter_GU = 170;
 int Filter_UJ = 190;
extern int target_input_points_input=0 ; 
 double StopLoss = 4000;  //this is for our BUY/SELL STOP SL 2.3 pips modify, can not put 200 pips, will lose !!!
 double Limit = 4000.0;    // 20 will cause order130 error
 double Distance_input = 4000; // not used !!!
 double NoTrade_Profit_Yet_StopLoss = 5000;  //30 pips
 double NoTrade_Profit_Yet_Limit = 5000;  // can not play with this , 20 should be ok!!!
 double NoTrade_Profit_Yet_Distance = 5000;
 double InTrade_Profit_Distance = 5000;
 double InTrade_Profit_StopLoss = 5000;       //2.3 pips
 double InTrade_Profit_Limit = 5000;
 bool Use_TrailingStep = TRUE;
 double Start_Trailing_At = 5000.0;  // org 2.9 meaning keep bid/ask Distance_input by x PIPS
 double TrailingStep = 50.0;        // org 0.3
 bool Use_Set_BreakEven = TRUE;
 double Set_BreakEvenAt = 5000.0;   // org 2.3
 double LockPips =  30.0;           // org 0.3
extern double MaxSpreadPlusCommission = 3000 ;//6.3 ok !
 double MaxSpreadPlusCommission_EU = 16.3;//0.6 ok !
 double MaxSpreadPlusCommission_GU = 16.0;//1.0 ok !
 double MaxSpreadPlusCommission_UJ = 16.0;//0.6 ok !

 double MaxSpread = 5000  ;//6.3 ok !
 double MaxSpread_EU = 12.6;//0.6 ok !
 double MaxSpread_GU = 12.0;//1.0 ok !
...
int total_orders_open_live;
  ...
  int Filter,target_input_points;
...
int init() 
    {
      ...
      Filter=Filter_input;
      target_input_points=target_input_points_input;
   ...
   if (symbol_name == "BTCUSD" || symbol_name == "BTCUSDc" ) 
           ...
           if(Filter_input==0) 
            Filter=10*skspread;
               ...
               if(Filter_input<100 ) 
                  Filter=Filter_input*skspread;
                
         if(target_input_points_input<100 ) 
            target_input_points=target_input_points_input*skspread;
...
// work with GS_digits 
   ...
   if(  (current_equity - brokerage_total ) <=0  )  // if current equity with brokerage is low 
      {
         if(total_orders_open_live>0)
            close_all_orders();
         return;
   ...
   if(  usable_equity   <= ( equity_allocated_touse - acceptable_equity_loss )  )  // if current equity with brokerage is low 
      {
         ...
         if(total_orders_open_live>0)
         {
         printf("## orders="+total_orders_open_live+"/"+OrdersTotal()+" usable_equity="+usable_equity+" <=  ( equity_allocated_touse - acceptable_equity_loss )="+( equity_allocated_touse - acceptable_equity_loss ));
   ...
   if (total_orders_open_live <= MaxTrades -1 && buyorsell != 0  && usable_equity   > ( equity_allocated_touse - acceptable_equity_loss -(4*lot_size_to_trade * average_spread_withcommision ) )  ) 
   {  // New BUY STOP & SELL STOP Orders placed here..
      ...
      */
      ...
      if(average_spread_withcommision > max_spreadandcommision)
         { 
         printf("order skip due to high maxspread commision ="+average_spread_withcommision+">"+max_spreadandcommision); 
         return ;
         }
...
int deinit()
   ...
   */
   ...
   printf("\n#Filter="+Filter+"Sprd="+DoubleToStr(Ask-Bid, GS_digits + 1)+" Pft=" + (current_equity-equity_at_start-brokerage_total )+" "+TimeToStr(first_order_time)+"-"+TimeToStr(last_order_time)+" Traded lots="+ int(total_traded_lots_tillnow)+"/"+int(lots_in_this_session)+ " Brkg="+brokerage_total +"/"+brokerage_current+"/"+ " EQT " +equity_at_start+"/"+total_invested_money+"/"+ equity_allocated_touse +" to "+ current_equity+"/"+(current_equity-brokerage_total) +"/"+ usable_equity +"sprd="+(Ask-Bid)+ " Pft=" + (current_equity-equity_at_start-brokerage_total )+ "###--- "  );
   printf("######################## END #####################");
   return(1);
   }
...
2022.02.15 17:34:05.867	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 

 Next Lot=1.00/20 Time Period=2022.02.01 09:04-2022.02.01 10:15 point=0.01 digit=2
 Orders#32/288 Traded_lots=156/26 Brkg=1078/168/ EQT 1000/1000/100 to 1091/13/13 Sprd=19.38 Pft=-987
 Equity min=13/801 max_equity=1000/1635 Free=1091.46
 Current Spread: 1938/5000|  
 AvgSprd: 19.38 brkg actual:0.000 Real avg spread+comision: 19.380  
 Params :Filter=19379 SL=3876 TP=3875 MaxTrades=10 Risk=0.1 MaxSpread=5000/3000 HL_df_lvl=1 Filter=235.54/193.79


2022.02.15 16:55:02.435	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter=12059Sprd=12.060 Pft=-1005 2022.02.01 01:46-2022.02.02 14:32 Traded lots=251/1 Brkg=1758/10/ EQT 1000/1000/100 to 1753/-5/-5sprd=12.06 Pft=-1005###--- 

2022.02.15 12:05:09.135	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter=9959 Sprd=9.960 Pft=11746 2022.02.01 00:17-2022.02.14 23:01 Traded lots=4140/4130 Brkg=28986/28910/ EQT 1000/200/100 to 41732/12746/11946 sprd=9.96 Pft=11746###--- 
2022.02.15 14:48:26.695	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter=17580Sprd=17.580 Pft=-1018 2022.02.01 09:04-2022.02.03 21:00 Traded lots=60/7 Brkg=424/54/ EQT 1000/1000/100 to 406/-18/-18sprd=17.58 Pft=-1018###--- 
2022.02.15 14:49:58.533	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter=18000Sprd=18.000 Pft=-1005 2022.02.01 09:04-2022.02.04 13:30 Traded lots=49/2 Brkg=349/19/ EQT 1000/1000/100 to 344/-5/-5sprd=18 Pft=-1005###--- 
2022.02.15 15:23:37.617	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter=11500Sprd=11.500 Pft=8104 2022.02.01 00:17-2022.02.14 22:13 Traded lots=2804/2633 Brkg=19634/18433/ EQT 1000/300/100 to 28738/9104/8404sprd=11.5 Pft=8104###--- 
2022.02.15 16:43:12.732	2022.02.14 23:59:48  sk5_btc_15feb22_1 BTCUSD,M1: 
#Filter