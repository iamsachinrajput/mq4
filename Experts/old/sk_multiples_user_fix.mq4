...
// ================== variables 
  ...
  int MagicNumber = 150115;
  string TradeComment = "SKRAJ";
  input int MaxTrades = 10;
  input double Risk = 10;  // org 60

  extern double StopLoss = 10;  //this is for our BUY/SELL STOP SL 2.3 pips modify, can not put 200 pips, will lose !!!

  //double MaxLots = 100000000.0;//100000.0;
  double MaxLots = 200.0;//100000.0;

  double NoTrade_Profit_Yet_StopLoss = 30;  //30 pips
  double InTrade_Profit_StopLoss = 60;       //2.3 pips

  double Distance = 30; // not used !!!
  ...
  double MaxSpreadPlusCommission = 6.3 ;//6.3 ok !
  ...
  double MaxSpread = 6.3  ;//6.3 ok !
  double MaxSpread_EU = 0.6;//0.6 ok !
  double MaxSpread_GU = 1.0;//1.0 ok !
...
int init() {
   ...
   if (EASymbol == "EURUSD") {MaxSpread  = MaxSpread_EU; Filter = Filter_EU;SL_multiplier=6 ;}
   if (EASymbol == "GBPUSD") {MaxSpread  = MaxSpread_GU; Filter = Filter_GU;SL_multiplier=6 ;}
   if (EASymbol == "USDJPY") {MaxSpread  = MaxSpread_UJ; Filter = Filter_UJ;SL_multiplier=4 ; }
...
// work with DIGITS 
   ...
   //if (count_184 /*== 0*/ <= MaxTrades -1 && Li_180 != 0 /*&& Ld_164 <= Gd_232*/ && NormalizeDouble(Ask - Bid, Digits) < NormalizeDouble(MaxSpread * pp, pd + 1) && f0_4()) 
   if (count_184 /*== 0*/ <= MaxTrades -1 && Li_180 != 0 && Ld_164 <= Gd_232 && NormalizeDouble(Ask - Bid, Digits) < NormalizeDouble(MaxSpread * pp, pd + 1) && f0_4()) 
   {  // New BUY STOP & SELL STOP Orders placed here..
      double mSLS = MarketInfo(Symbol(),MODE_STOPLEVEL)*1.2;
      printf("mSLS"+mSLS+"order type ( 1 for sell , -1 for buy , 2 for both )= "+Li_180) ;

      if (Li_180 == -1 || Li_180 == 2 ) 
      {
         price_24 = NormalizeDouble(Ask + Gd_240, G_digits_192);
         //price_60 = NormalizeDouble(price_24 -  3 * StopLoss * Point, G_digits_192);
         //price_60 = NormalizeDouble(Bid -  SL_multiplier *  StopLoss * Point, G_digits_192);
         price_60 = NormalizeDouble(Bid -  mSLS *Point , G_digits_192);
         ticket_20 = OrderSend(Symbol(), OP_BUYSTOP, Ld_204, price_24, G_slippage_264, price_60, 0, TradeComment, MagicNumber, 0, Lime);
         //ticket_20 = OrderSend(Symbol(), OP_BUYSTOP, Ld_204, price_24, G_slippage_264, 0, 0, TradeComment, MagicNumber, 0, Lime);
         if (ticket_20 <= 0) {
            error_8 = GetLastError();
            Ls_12 = ErrorDescription(error_8);
            ...
            total_traded_lots_tillnow+=Ld_204;
            }
      ...
      if (Li_180 == 1 || Li_180 == 2 ) {
          price_24 = NormalizeDouble(Bid - Gd_240, G_digits_192);
          //price_60 = NormalizeDouble(price_24 +  3 *StopLoss  * Point, G_digits_192);
          //price_60 = NormalizeDouble(Ask + SL_multiplier *   StopLoss  * Point, G_digits_192);
          price_60 = NormalizeDouble(Ask + mSLS * Point, G_digits_192);
          //price_60 = NormalizeDouble(price_24 +  StopLoss  * Point, G_digits_192);
          ticket_20 = OrderSend(Symbol(), OP_SELLSTOP, Ld_204, price_24, G_slippage_264, price_60, 0, TradeComment, MagicNumber, 0, Orange);
          //ticket_20 = OrderSend(Symbol(), OP_SELLSTOP, Ld_204, price_24, G_slippage_264, 0, 0, TradeComment, MagicNumber, 0, Orange);
          if (ticket_20 <= 0) {
            error_8 = GetLastError();
            Ls_12 = ErrorDescription(error_8);
   ...
   string Ls_212 = "\n\n Equity from "+ opening_balance+" to " + AccountEquity() +" wihtout brokerage = " + (AccountEquity()-( total_traded_lots_tillnow * brokerage_perlot )  ) + " min="+min_equity  +" max_equity="+max_equity
