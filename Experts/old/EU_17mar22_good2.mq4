...
input string info3="----SL=just for stop orders ";
extern double StopLoss = 10;  //this is for our BUY/SELL STOP SL 2.3 pips modify, can not put 200 pips, will lose !!!
extern double NoTrade_Profit_Yet_StopLoss = 10;  //30 pips
extern double InTrade_Profit_StopLoss = 10;       //2.3 pips
...
extern int target_input_points=10 ; 

extern int Filter_input = 1000;
int Filter=Filter_input;
double current_spread;
extern int Filter_EU = 40; //org 140
 int Filter_GU = 170;
 int Filter_UJ = 190;
...
extern  string info1="----Distance=modifying SL of Active orders to trail profits . ";
extern  double sl_trail_active_orders_input = 2; // not used !!! 30
extern  double NOPROFIT_sl_trail_active_orders_input = 10;  // 20 
extern  double WITHPROFIT_sl_trail_active_orders_input = 10;  // 30
 
extern string info2="----Limits=open price calculation of pending orders";
extern  double Limit = 1;    // 20 will cause order130 error  //23
extern  double NoTrade_Profit_Yet_Limit = 10;  // can not play with this , 20 should be ok!!!  //22
extern  double InTrade_Profit_Limit = 10;  //25
...
extern double Start_Trailing_At = 2.9;  // org 2.9 meaning keep bid/ask sl_trail_active_orders_input by x PIPS
...
extern bool lots_auto_managed = False;
...
int init() 
    {
     ...
     MaxTrades=MathMax(1,MaxTrades_input );
     Risk=MathMax(1,Risk_input );
...
//################  Main calculation to trade decision check if high movement is there 
   ...
   printf("Got Peak :"+highlow_diff/GS_point+">"+given_filter_normalized/GS_point);
       ...
       TradeComment="peak:"+(highlow_diff/GS_point)+"PR="+profit_total;
            ...
            if (OrderSymbol() == Symbol()) 
            {
               ...
               case OP_BUY:
                 ...
                 TotalProfit = (Bid - OrderOpenPrice()) /Point/Dfactor_basedondigits;
                  ...
                  //Print("*****TotalProfit=",TotalProfit, ", sl_trail_active_orders_input=", sl_trail_active_orders_input, ", Bid - sl_trail_active_orders_input", SL_tobeused_inorder, ", Stoploss=",order_SL_normalized );
                  
                  if ( order_SL_normalized == 0.0 ||  SL_tobeused_inorder < order_SL_normalized )
                  {
                  //printf(" no need to modify SL ");
                  break;   // Bid - sl_trail_active_orders_input > Stoploss 
                  }
