                    ...
                    // while previous level is diffirent then current so level changed.  
                        ...
                        if(sklevel_current > 0) // mkt going up so book buy profits 
                            ...
                            // while going towards rally Above the starting point 
                                ...
                                {
                                    skdebug("sklevel upward newposition : levels from "+sklevel_prev+" to "+sklevel_current ,dbgcheck1);
                        ...
                        if(sklevel_current < 0)   // mkt going down so book profit from sell 
                            ...
                            // while going towards rally below the starting point 
                                ...
                                {
                                    skdebug("sklevel downward newposition : levels from "+sklevel_prev+" to "+sklevel_current ,dbgcheck1);
...
// Remote status update message
   void send_update_ontick()
    {
        if(update_status && user_status=="registered") 
            {  
            datetime sknow=TimeCurrent();
            //int skcurrent=TimeMinute(TimeCurrent());   
            int skcurrent=TimeHour(TimeCurrent());   
            if(skcurrent!=msg_last_sent )   // msg_last_sent is global variable 
                {
                ...
                send_telegram_msg("#"+AccountNumber()+" "+ balance_info_string()+" AT:"+sknow);
                }
            }
    }
...
// License check functions . 
    ...
    void set_basic_params_based_on_symbol()
        {
    ...
    void OnTick()
        {
            if(letswork && user_allowed )
            {
                   ...
                   if(executetrades && good_day_to_work() )
                     {
