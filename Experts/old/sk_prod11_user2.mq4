...
// work with GS_digits 
   ...
   if((AccountEquity()-( total_traded_lots_tillnow * brokerage_perlot )  ) < 0  )
   {
   //printf("money is finished can not further ");
   //deinit();
   return(0);
   } 
   if (total_orders_open_live <= MaxTrades -1 && buyorsell != 0 && average_spread_withcommision <= max_spreadandcommision && NormalizeDouble(Ask - Bid, GS_digits) < NormalizeDouble(MaxSpread * pp, pd + 1) && workinghours_check_function()) 
   {  // New BUY STOP & SELL STOP Orders placed here..
      ...
      printf(" Orders="+ OrdersTotal() +"/"+ OrdersHistoryTotal()+"Profit = " + (AccountEquity()- opening_balance ) + " after brokerage = " + (AccountEquity()- opening_balance -( total_traded_lots_tillnow * brokerage_perlot ) ) );
