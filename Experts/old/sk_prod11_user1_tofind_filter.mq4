...
// ================== variables 
 string yourtelegramchannel="thisisforsachin"; 
 string yourlicensekey="thisisforsachin";
 datetime licesevaliddate;
 bool send_telgram_message_flag=False;
int old_msg_sent_time=0;
string equity_filter;
 extern int MaxTrades = 20;
 extern double Risk = 3;  // org 60
 extern int brokerage_perlot=5;

  double total_traded_lots_tillnow=0;
extern int opening_balance=100;
...
extern  int target_input_points=100 ; // if you want to put the target 
...
int init() 
    {
        ...
        double GS_allowed_SL_size_for_symbol = MarketInfo(Symbol(),MODE_STOPLEVEL);
...
//sk_prod11_user1_tofind_filter EURUSDc,M1: equity_filter=Filter: 120=677.43 125=677.43 130=677.43 135=677.43 140=677.43 145=677.43 150=677.43 155=677.43 160=677.43 165=677.43 170=677.43 175=677.43 180=677.43 185=677.43 190=677.43
            ...
            if (OrderSymbol() == Symbol()) 
            {
               ...
               case OP_SELL:
                 ...
                 TotalProfit = (OrderOpenPrice()- Ask)/Point/Dfactor_basedondigits;
                  ...
                  given_limit_normalized = NormalizeDouble(Limit * GS_point, GS_digits); 
                  order_SL_normalized = NormalizeDouble(OrderStopLoss(), GS_digits);
                  //SL_tobeused_inorder = NormalizeDouble(Ask + given_limit_normalized, GS_digits);
               ...
               case OP_BUYSTOP:
                  ...
                  SL_tobeused_inorder = NormalizeDouble(price_tobeused - given_limit_normalized, GS_digits);
                  ResetLastError();
                  ...
                  error_code = GetLastError();
                  error_details = ErrorDescription(error_code);
                  Print("BUYSTOP Modify order#"+OrderTicket()+ " Error Code: " + error_code + " Message: " + error_details + " OP: " + DoubleToStr(price_tobeused, GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                  //Print("Modify error order#"+ OrderTicket()+" type: "+open_order_type+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                  break;
               case OP_SELLSTOP:
                  current_order_price = NormalizeDouble(OrderOpenPrice(), GS_digits);
                  ...
                  if(boolvar_modify_done) 
                  {
                  //Print("success SELLSTOP Modify order#"+OrderTicket() );
                  //Print("Modify : "+open_order_type+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                     break;
                  }
                  error_code = GetLastError();
                  error_details = ErrorDescription(error_code);
                  //Print("SELLSTOP Modify order#"+OrderTicket()+ " Error Code:" + error_code + " Message: " + error_details );
                  Print("Modify error  order#"+ OrderTicket()+" type: "+open_order_type+" OP: " + DoubleToStr(OrderOpenPrice(), GS_digits) + " SL: " + DoubleToStr(SL_tobeused_inorder, GS_digits) + " Bid: " + DoubleToStr(Bid, GS_digits) + " Ask: " + DoubleToStr(Ask, GS_digits));
                  break;
...
// work with GS_digits 
   ...
   + "  \n Params :Filter="+Filter+" SL="+SL_points_touse+" TP="+TP_tobeused_inorder +" MaxTrades="+ MaxTrades +" Risk="+ Risk + " Limit="+Limit
   ;
...
int deinit()
   ...
   printf("Params :Filter="+Filter+" SL="+SL_points_touse+" TP="+TP_tobeused_inorder +" MaxTrades="+ MaxTrades +" Risk="+ Risk );
   printf("equity_filter="+equity_filter);
return(0);
   }