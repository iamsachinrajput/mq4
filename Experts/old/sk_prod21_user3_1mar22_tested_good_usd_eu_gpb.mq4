...
extern bool trade_with_profits=False;
...
extern int Filter_UJ = 190;
 ...
 double Limit = 23.0;    // 20 will cause order130 error
 double NoTrade_Profit_Yet_Limit = 22;  // can not play with this , 20 should be ok!!!
 double InTrade_Profit_Limit = 25;
...
extern double FixedLots = 0.01;//0.1;
 ...
 double MaxSpreadPlusCommission = 16.3 ;//6.3 ok !
extern double MaxSpreadPlusCommission_EU = 1.3;//0.6 ok !
 double MaxSpreadPlusCommission_GU = 16.0;//1.0 ok !
 double MaxSpreadPlusCommission_UJ = 16.0;//0.6 ok !

 double MaxSpread = 12.6  ;//6.3 ok !
...
//################  Main calculation to trade decision check if high movement is there 
 ...
 highlow_diff=iHigh(NULL, 0, higest_bar) - iLow(NULL, 0, lowest_bar) ; 
   ...
   peak_name=""+TimeDay(timenow)+TimeMonth(timenow)+"."+TimeHour(timenow)+TimeMinute(timenow);
...
//// Calculations for peaks 
 ...
 //if(profit_total-profit_till_last_peak!=0)
 {
   ...
   + "\n\n Orders#"+OrdersTotal()+"/"+total_orders_executed+" Traded_lots="+ int(total_traded_lots_tillnow)+"/"+int(lots_in_this_session)+ " Brkg="+brokerage_total +"/"+brokerage_current+"/"
   + " EQT " +equity_at_start+"/"+total_invested_money+"/"+ equity_allocated_touse +" to "+ current_equity+"/"+(current_equity-brokerage_total) +"/"+ usable_equity +" Sprd="+((Ask-Bid)/GS_point)+ " Pft=" + profit_total 
   ...
   + "  \n Params :Filter="+Filter+" SL="+SL_points_touse+" TP="+ target_input_points+" MaxTrades="+ MaxTrades +" Risk="+ Risk +" HL_df_lvl="+ highlow_diff_level
   + " MaxSpread="+DoubleToStr(MaxSpread, Digits - pd)+"/"+DoubleToStr(MaxSpreadPlusCommission, Digits - pd)+" HL_df_lvl="+ highlow_diff_level+" Filter="+(highlow_diff/GS_point)+"/"+ (given_filter_normalized /GS_point ) 
...
int fresh_invest_everytime_function()
   {
      ...
      if(  current_equity < equity_at_start)
        ...
        else
        {
            investment_counter=1;
            total_invested_money=(equity_allocated_touse*investment_counter);
            if( current_equity > equity_at_start && trade_with_profits==True )
               {
               total_invested_money=( equity_allocated_touse + (profit_total)/(current_equity/equity_at_start));
               //MaxTrades=MathMin(100,current_equity/(equity_at_start/MaxTrades));
               //Risk=MathMin(100,current_equity/(equity_at_start/Risk) );
               MaxTrades=MathMin(100,MathMax(MaxTrades,total_invested_money/10));
               Risk=MathMin(1000,MathMax(Risk,total_invested_money/10) );
              riskpercent = Risk / 100.0;
               }
...
int deinit()
   ...
   */
   ...
   printf("peak_details#"+peaks_count+"="+peak_details);
   printf("######################## END #####################");
   return(1);
   }
