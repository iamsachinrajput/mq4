      ...
      if(barcountnow>=lastbarscount+barsgap || total_orders_open_live==0  ) 
       {
        ...
        if(action_on_xb4d("now")=="buy" )
         action_decided="buy";
        else if( action_on_xb4d("now")=="sell"  )
         action_decided="sell";
        ...
        if(action_decided=="buy" || action_decided=="sell")
         printf("reverse,action_decided,total_orders_open_live,same_position_ok,last_action:"+reverse+","+action_decided+","+total_orders_open_live+","+same_position_ok+","+last_action ) ;
...
string action_on_sar(string when)
 ...
 if(when=="now")
 {
    if( indsar < Close[0] && indsar_prev > Close[1] )
      returnvalue="buy";
    else if( indsar > Close[0] && indsar_prev < Close[1])
      returnvalue="sell";
...
string action_on_xb4d(string when)
   {
   string returnvalue="none";
   double indxb4d=iCustom(Symbol(),Period(),"xb4d",1,0);
   double indxb4d_prev=iCustom(Symbol(),Period(),"xb4d",1,1);
