...
extern double max_lot_size=20;
extern double lots_multiplier=2;
extern double per_lot_profit_usd=.01;
extern string skline="######### just upto here " ;
...
extern bool same_position_ok=True;
...
double min_profit;
double indxb4d_prev,indxb4d,lastclose_price;
int barscount_last_check=0;
double last_profit=0;
double last_closed_order_profit=0,last_order_profit=0;
   ...
   if((total_orders_open_active)==0  )
      {
         ...
         if((current_session_profit)!=0)
            ...
            last_profit=current_session_profit;
            if(current_session_profit>0)
               trades_inprofit++;
            if(current_session_profit<0)
               trades_inloss++;
...
void display_tasks()
{
   ...
   +" Gap="+gapinput_pips +" MNP="+min_profit
   +"\n#"+sessionid+"(P="+trades_inprofit+" L="+trades_inloss+") profits:"+session_list
   +"\n reverse="+reverse+" mavol-S="+digit1(mavolshortval)+" mavol-L="+digit1(mavollongval)+ " last_closed_order_profit="+last_closed_order_profit
   ) ;
...
double calculate_lots_to_trade()
{
  ...
  if(lots_multiplier==-1) // will use add . 
      lot_size_to_trade=lot_size_to_trade+lots_to_start_input*(total_orders_open_active+1);
  ...
  if(lots_multiplier==-2) // will use add . 
   ...
   if(total_orders_open_live==0 && last_profit>0)
         lot_size_to_trade=lots_to_start_input;
 ...
 min_profit=(per_lot_profit_usd*lot_size_to_trade/Point()); 
 
 lot_size_to_trade=MathMin(max_lot_size,digit2(lot_size_to_trade));
 
 //printf("lots to trade next = "+lot_size_to_trade);
      ...
      if(barcountnow>=lastbarscount+barsgap || total_orders_open_live==0  ) 
       {
        ...
        string action_decided="none";
        //if(action_on_rsi(14,20,60,80,"any")=="buy" && action_on_sar("now")=="buy" )
        if(barcountnow!=barscount_last_check)
        {
        string action_found=action_on_sar("once");
        if(total_orders_open_live==0)
         action_found=action_on_sar("continuous");
        barscount_last_check=barcountnow;
        if(action_found=="buy" )
         action_decided="buy";
        else if( action_found=="sell"  )
         action_decided="sell";
         
           if(reverse_input==True && last_order_profit<0 && total_orders_open_live<=1 && action_decided!="none" )
           {
            printf("will change reverse from "+reverse +" last_profit="+last_order_profit);
               if(reverse==True)
                  reverse=False ;
                  else
                     reverse=True;
            }
         printf("last_closed_order_profit="+last_closed_order_profit+" reverse:"+reverse+" action_decided:"+action_decided+" total_orders_open_live:"+total_orders_open_live+" action_decided:"+action_decided+" last_action:"+last_action+" last_profit:"+last_profit+" barcountnow:"+barcountnow+" barscount_last_check:"+barscount_last_check );
         
         }
        
       // printf("skaction = "+action_decided);
        ...
        //if(reverse_input==True && action_on_atr(12,20)=="zigzag")
        if(action_decided=="buy" || action_decided=="sell")
         printf("last_closed_order_profit="+last_closed_order_profit+" reverse:"+reverse+" action_decided:"+action_decided+" total_orders_open_live:"+total_orders_open_live+" action_decided:"+action_decided+" last_action:"+last_action+" last_profit:"+last_profit+" barcountnow:"+barcountnow+" barscount_last_check:"+barscount_last_check );
...
string action_on_rsi(int rsibars,int rsidownlevel,int rsimidlevel , int rsiuplevel,string whichtype)
   {
   string returnvalue="none";
    double indrsi=iRSI(NULL,0,rsibars,PRICE_CLOSE,0);
    if( ( indrsi>rsimidlevel && indrsi<rsiuplevel && whichtype!="onlyover" ) || ( indrsi<rsidownlevel && whichtype!="onlymid" ) )
      returnvalue="buy";
    else if( ( indrsi<rsimidlevel && indrsi>rsidownlevel && whichtype!="onlyover" ) ||  ( indrsi>rsiuplevel && whichtype!="onlymid" ) )
      returnvalue="sell";
   return(returnvalue);
   }
   
string action_on_sar(string when)
   {
   string returnvalue="none";
 double indsar=iSAR(NULL,0,0.02,0.2,0);
 double indsar_prev=iSAR(NULL,0,0.02,0.2,1);
 double indsar_prev2=iSAR(NULL,0,0.02,0.2,2);
 
 //printf("##sksar :"+indsar+ "close_0="+Close[0]+" prv="+indsar_prev+" close_1="+Close[1]+" prv2="+indsar_prev2+" close_2="+Close[2]); 
 
 if(when=="continuous")
 {
    if( indsar_prev < Close[1] )
      returnvalue="buy";
    else if( indsar_prev > Close[1])
      returnvalue="sell";
  }
 if(when=="once")
 {
    if( indsar_prev < Close[1] && indsar_prev2 > Close[2] )
      returnvalue="buy";
    else if( indsar_prev > Close[1] && indsar_prev2 < Close[2])
      returnvalue="sell";
...
string action_on_highlow(int dist)
   {
   string returnvalue="none";
...
string action_on_xb4d(string when)
   {
   string returnvalue="none";

   indxb4d_prev=indxb4d;
   indxb4d=iCustom(Symbol(),Period(),"xb4d",27,0,0);
     printf("on close bar : indxb4d="+indxb4d+" indxb4d_prev="+indxb4d_prev+" close_0="+Close[0]+" close_1="+Close[1]);
 ...
 if(when=="once")
 {
    if( indxb4d >0 && indxb4d_prev <0 )
      returnvalue="buy";
    else if( indxb4d <0 && indxb4d_prev >0 )
      returnvalue="sell";      
  }
  
  if(returnvalue!="none")
      printf("indxb4d="+indxb4d+" indxb4d_prev="+indxb4d_prev);
...
else if ( lotsneeded>=max_lot_size ) 
    {
...
else if(take_profit_usd_input==0 && ( current_session_profit_now > min_profit ) && ( (actiontodo=="buy" && total_open_lots_sell>0 ) || (actiontodo=="sell" && total_open_lots_buy>0 ) )  )
      {
...
remaininglots=(MathMin(max_lot_size,remaininglots));
          ...
          { 
            if ( OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber ) 
            {
                  ...
                  if(OrderType() == whichorders || whichorders == 100  )
                   {
                     last_closed_order_profit=OrderProfit();
                     printf("before closing order#"+ OrderTicket() +" lots="+ OrderLots()+" pr="+last_closed_order_profit);
                     if( !OrderClose(OrderTicket(), OrderLots(), Bid, G_slippage_264 , clr_confirm_order_close))
                     {  printf(" order closing second try") ;
