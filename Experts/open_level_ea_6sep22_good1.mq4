...
extern double lots_to_start_input=0.05;
extern bool same_position_ok=True;
extern double lots_multiplier=1;
extern double set_take_profit_pip_input=50;
extern double set_sl_pip_input=50;
extern double sl_usd_input=0;
extern bool chang_gap_for_martingle=True;
extern double take_profit_usd_input=100;
      ...
      if(total_orders_open_live<=max_orders_depth )
       {
        ...
        barcountnow=Bars;
            ...
            if(buy_sell_with_adionly()=="buy" && ( last_action==OP_SELL || total_orders_open_live==0 || same_position_ok==True ) && barcountnow!=lastbarscount)
               {  
                  if(reverse==True)
                     {action1=OP_SELL;printf("will do sell  of "+lot_size_to_trade+" lots lastaction was "+last_action +" Ask:"+ Ask   ); }
                     else
                     {action1=OP_BUY;printf("will do buy  of "+lot_size_to_trade+" lots lastaction was "+last_action +" Bid:"+ Bid  ) ; }
                lastbarscount=barcountnow;

               }
            //if(Ask < selllimitprice  )
            if(buy_sell_with_adionly()=="sell" && ( last_action==OP_BUY || total_orders_open_live==0 || same_position_ok==True  ) && barcountnow!=lastbarscount )
               {  
                  if(reverse==True)
                     { action1=OP_BUY; printf("will do buy  of "+lot_size_to_trade+" lots lastaction was "+last_action +" Bid:"+ Bid ) ; }
                     else
      ...
      if(total_orders_open_live<=max_orders_depth )
       {
        ...
        barcountnow=Bars;
            ...
            if(closeprice > buyprice && openprice < buyprice && ( last_action==OP_SELL || total_orders_open_live==0 || same_position_ok==True ) && barcountnow!=lastbarscount)
               {  
                  if(reverse==True)
            ...
            //if(Ask < selllimitprice  )
            if(closeprice < sellprice && openprice > sellprice && ( last_action==OP_BUY || total_orders_open_live==0 || same_position_ok==True  ) && barcountnow!=lastbarscount )
               {  
                  if(reverse==True)
                     { action1=OP_BUY; printf("will do buy  of "+lot_size_to_trade+" lots lastaction was "+last_action +" Bid:"+ Bid +">"+ buyprice+" cp="+closeprice +" op="+openprice ) ; }
                     else
